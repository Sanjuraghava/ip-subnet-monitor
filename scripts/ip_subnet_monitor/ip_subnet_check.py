...
def render_report(results: list[ScanResult]) -> str:
    ...
    lines.append(f"🔴 Critical Subnets : {critical_count}")
    lines.append(f"🟡 Low Capacity     : {warning_count}")
    lines.append(f"🟢 Healthy Subnets  : {healthy_count}")
    ...
    if total_conflicts:
        lines.append(
            f"❌ IP Conflicts Detected : {total_conflicts}"
        )

        for result in results:
            for item in result.conflicts:
                lines.append(
                    f"   - {item['ip']} ({result.cidr})"
                )

    else:
        lines.append("✅ No IP conflicts detected")

    if total_missing:
        lines.append(
            f"⚠️ Missing Expected IPs : {total_missing}"
        )

    else:
        lines.append("✅ No missing expected IPs")

    lines.append("")
    lines.append("🤖 Jenkins IP Subnet Monitor")
