...
def normalize_records(records: list[dict]) -> dict[str, dict]:
    ...
    for record in records:
        ...
        if expected not in {
            None,
            ScanStatus.IN_USE.value,
            ScanStatus.FREE.value,
        }:
...
def scan_subnet(
    ...
    return ScanResult(
        name=subnet["name"],
        lab_name=subnet.get("lab_name", "UNKNOWN-LAB"),
        cidr=str(network),
    ...
    for result in results:
        ...
        if len(result.free) < 20:
            ...
            critical_subnets.append(
                f"🔴 {result.cidr} has only "
                f"{len(result.free)} free IPs remaining."
            )
...
def should_notify(
    ...
    if notify_on != "conflicts_only":
        raise ValueError(
            f"Invalid notify_on value: {notify_on}"
        )

    return any(
        result.conflicts or result.missing
        for result in results
    )
    ...
    try:
        with urllib.request.urlopen(
            ...
            if response.status >= 300:
                raise RuntimeError(
                    f"Webex returned HTTP {response.status}"
                )
        ...
        raise RuntimeError(
            f"Webex returned HTTP {exc.code}: {body}"
        ) from exc
...
def load_config(path: Path) -> dict:
    ...
    if "subnets" not in config:
        raise ValueError(
            "Config must contain a subnets array"
        )
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Scan configured subnets and report IP usage."
    )

    parser.add_argument(
        "--config",
        default=Path(__file__).with_name("subnets.json"),
        type=Path
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print report without sending Webex"
    )
    ...
    if not token or not room_id:
        ...
        print(
            "WEBEX_BOT_TOKEN and WEBEX_ROOM_ID are required.",
            file=sys.stderr
        )
